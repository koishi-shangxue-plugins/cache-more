...
// 将对象数组序列化为 INI 字符串
function stringifyIni(data: any[]): string
{
  let result = '';
...
function parseIni(content: string): any[]
{
  ...
  for (const line of lines)
  {
    ...
    if (currentObject)
    {
      const keyValueMatch = trimmedLine.match(/^\s*([^=]+?)\s*=\s*(.*)\s*$/);
      if (keyValueMatch)
      {
...
class IniDBDriver extends Driver
{
  static name = 'inidb';
  ...
  constructor(public ctx: Context, public config: IniDBDriver.Config)
  {
    super(ctx, config);
    this.logger = ctx.logger(IniDBDriver.name);
      ...
      for (const file of files)
      {
        if (file.endsWith('.ini'))
        {
          ...
          try
          {
            ...
            if (data) this.store[tableName] = parseIni(data);
          } catch (err)
          {
  ...
  private async flushTable(table: string)
  {
    ...
    const tablePath = resolve(this._path, `${table}.ini`);
    try
    {
      ...
      {
        // 如果表有数据，则写入文件
        await fs.writeFile(tablePath, stringifyIni(tableData));
      } else
      {
...
namespace IniDBDriver
{
  ...
  export const Config: Schema<Config> = Schema.object({
    path: Schema.path({
      filters: ['directory'],
      allowCreate: true,
    }).description('数据库目录的路径。').default('data/database/inidb'),
  });
}

export default IniDBDriver;