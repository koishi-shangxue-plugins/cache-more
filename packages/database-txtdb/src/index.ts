...
class TxtDBDriver extends Driver
{
  static name = 'txtdb';
  ...
  constructor(public ctx: Context, public config: TxtDBDriver.Config)
  {
    super(ctx, config);
    this.logger = ctx.logger(TxtDBDriver.name);
  ...
  private async flushTable(table: string)
  {
    ...
    const tablePath = resolve(this._path, `${table}.txt`);
    try
    {
      ...
      {
        // 如果表有数据，则写入文件
        await fs.writeFile(tablePath, JSON.stringify(tableData));
      } else
      {
...
namespace TxtDBDriver
{
  ...
  export const Config: Schema<Config> = Schema.object({
    path: Schema.path({
      filters: ['directory'],
      allowCreate: true,
    }).description('数据库目录的路径。').default('data/database/txtdb'),
  });
}

export default TxtDBDriver;